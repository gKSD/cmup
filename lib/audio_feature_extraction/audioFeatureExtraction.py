...
def stZCR(frame):
    ...
    #zcrs = librosa.zero_crossings(frame, pad=True)
    #print "KSD To find the total number of zero crossing => " + str( numpy.sum(numpy.abs(zcrs)) / 2 )
...
def stSpectralCentroidAndSpread(X, fs):
    ...
    # Centroid:
    C = (NUM / DEN)
    # print "KSD Spectral Centroid, C: " + str(C)
    # print "KSD Spectral Centroid, librosa.feature.spectral_centroid(y, sr=fs): " + str(librosa.feature.spectral_centroid(X, sr=fs))
    ...
    C = C / (fs / 2.0)
    S = S / (fs / 2.0)
    #print "KSD Spectral Centroid, C1: " + str(C)
...
def mtFeatureExtraction(signal, Fs, mtWin, mtStep, stWin, stStep):
    ...
    #print "KSD number of short time features: " + numOfFeatures
    numOfStatistics = 2
...
def fileWavFeatureExtraction(fileName, mtWin, mtStep, stWin, stStep, computeBEAT=False):
    """
    ...
    if not fileName.endswith('.wav'):
        raise Exception("Error: wrong file format, '.wav' expected!")

    print "[AUDIO_FEATURE_EXTRACTION] Analyzing file: " + fileName
    if os.stat(fileName).st_size == 0:
        print "[AUDIO_FEATURE_EXTRACTION] EMPTY FILE"
        raise Exception("Error: empty file")
    [Fs, x] = audioBasicIO.readAudioFile(fileName)            # read FILE 
    t1 = time.clock()        
    x = audioBasicIO.stereo2mono(x)                          # convert stereo to mono                
    if x.shape[0] < float(Fs)/10:
        print "[AUDIO_FEATURE_EXTRACTION] AUDIO FILE TOO SMALL"
        raise Exception("Error: audio file too small")
    ...
    processingTime = (t2 - t1)# / duration
    print "[AUDIO_FEATURE_EXTRACTION] Feature extraction processing time: {0:f}".format(processingTime)
...
# ------------------------ EXTENSION ---------------------------
def dirsWavFeatureExtractionExtensionImpl(allMtFeatures, realFileNames, dirName, mtWin, mtStep, stWin, stStep, computeBEAT=False):
    """
    This function extracts the mid-term features of the WAVE files of a particular folder.
...
def dirsWavFeatureExtractionExtension(resultFeatures, resultFileNames, dirNames, mtWin, mtStep, stWin, stStep, computeBEAT=False):
    '''
    Same as dirWavFeatureExtraction, but instead of a single dir it takes a list of paths as input and returns a list of feature matrices.
    EXAMPLE:
    [features, classNames] =
